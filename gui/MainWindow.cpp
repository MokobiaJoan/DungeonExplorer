      ...
      )
{
    ...
    if (playerClass_ == "Warrior") {
        showStory({
            "Born of steel and honor, you awaken amid the clang of swords.",
            "Your scars remind you of battles past, but a new threat looms."
        });
    } else if (playerClass_ == "Mage") {
        showStory({
            "Arcane energy crackles around you as you open your eyes.",
            "Runes glow on the walls and a tome lies open at your feet."
        });
    } else {
        showStory({
            "Silent as a shadow, you stir in the gloom of a narrow alley.",
            "Your keen senses detect danger and opportunity both."
        });
    }
    showStory({
...
void MainWindow::setupUi()
{
    ...
    itemInput_=new QLineEdit; itemInput_->setPlaceholderText("Item name");
...
{
    for(const QString &line:story) appendLog(line);
}
...
void MainWindow::refreshInventory()
{
    ...
    for(const QString &it:inventory_) inventoryList_->addItem(it);
}
...
void MainWindow::onAttack()
{
    ...
    else{int edmg=10;playerHp_=std::max(0,playerHp_-edmg);
...
void MainWindow::onSpecial()
{
    ...
    if(enemyHp_==0)appendLog("Enemy defeated!");
    else{int edmg=15;playerHp_=std::max(0,playerHp_-edmg);
...
void MainWindow::onUseItem()
{
    ...
    }else{appendLog("Cannot use that item now.");return;}
